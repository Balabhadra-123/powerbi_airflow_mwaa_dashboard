...
Email Changes (v1.2.0)
----------------------
- Validation Summary section removed
- Cluster Information trimmed to: Name, ID, State only
- Footer detail (Environment, Region, Generated) removed; kept automated notice
- Report heading includes cluster name and tick/cross symbol
- Subject line includes tick/cross symbol
...
def get_report_time():
    """Returns report timestamp in UTC."""
...
def cluster_information_rows(result):
    """Returns only the three core cluster fields for the email."""
    return [
...
def build_email_subject(result):
    """
    Builds the email subject line with a tick or cross symbol.

    HEALTHY          : ✅ SUCCESS | EMR Healthy | <cluster> | <timestamp>
    UNHEALTHY        : ❌ CRITICAL | EMR Unhealthy | <cluster> | <timestamp>
    CLUSTER_NOT_FOUND: ❌ CRITICAL | EMR Cluster Not Found | <cluster> | <timestamp>
    """
...
def build_email(result, context=None):
    ...
    if result["overall_status"] == STATUS_HEALTHY:
        ...
        banner_colour = RED
        status_symbol = "\u274C"        # ❌
        status_label = "Cluster Not Found"
        summary = (
    ...
    # Banner heading: ✅ EMR medibank-prod-emr-cluster Health Report - Healthy
    banner_heading = f"{status_symbol} EMR {cluster_name} Health Report - {status_label}"
...
<tr><td>
<div style="background:{banner_colour};padding:25px;color:white;font-size:24px;font-weight:bold;">
{banner_heading}
</div>
<p style="font-size:15px;margin-top:20px;line-height:24px;">{summary}</p>
"""

    # Cluster Information — Name, ID, State only
    html += build_table("Cluster Information", ["Property", "Value"], cluster_information_rows(result))

    # Node Capacity — skipped when cluster not found
    if result["overall_status"] != STATUS_CLUSTER_NOT_FOUND:
        html += build_table(
            "Node Capacity",
    ...
    # Recommended Actions — only shown for non-healthy
    ...
    # Footer — automated notice only, no environment/region/generated details
    html += f"""
